...
'''
        ...
        log_result = subprocess.run([
            ...
            "--since", "30 seconds ago", "--no-pager", "-n", "5"
        ...
        script_files = list(Path("/home/hiro/workspace/WSL-kernel-watcher").glob("docker_notify_*.sh"))
        if not script_files:
...
'''
        ...
        click_result = subprocess.run([
            "docker-compose", "run", "--rm",
            "wsl-kernel-watcher",
            "uv", "run", "python", "-c", click_test_script
        ], cwd=project_root, capture_output=True, text=True, timeout=60)
        ...
        print("\n📋 通知確認手順:")
        print("   1. 'クリックテスト - WSL Kernel Watcher' ダイアログが表示されたか確認")
        print("   2. ダイアログの 'OK' ボタンをクリック")
        print("   3. 通知が表示されない場合は 'n' を入力")
        print("   ※ 現在はMessageBoxダイアログ形式（Toast通知ではありません）")
        
        response = input("\n❓ 通知ダイアログが表示され、OKボタンをクリックできましたか？ (y/N): ").strip().lower()
...
def troubleshoot_notification_issues():
    ...
    try:
        ...
        script_path = f"/home/hiro/workspace/WSL-kernel-watcher/docker_notify_diag_{timestamp}.sh"
...
'''
        ...
        if not os.path.exists(script_path):
            ...
            # 診断通知の確認
            diag_response = input("\n❓ '診断テスト - WSL Kernel Watcher' Toast通知が表示されましたか？ (y/N): ").strip().lower()
...
if __name__ == "__main__":
    ...
    print("   Docker内部 → ファイル作成 → systemd検知 → 通知実行 → クリック確認 → クリーンアップ")
