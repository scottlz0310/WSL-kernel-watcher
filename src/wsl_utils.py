...
class WSLUtils:
    ...
    def get_current_kernel_version(self) -> Optional[str]:
        ...
        try:
            ...
            error_msg = "WSLが見つかりません。WSLがインストールされているか確認してください"
            logger.error(error_msg)
            raise WSLCommandError(error_msg)
    ...
    def _parse_version(self, version_string: str) -> Tuple[int, ...]:
        ...
        try:
            ...
            if "invalid literal for int()" in str(e):
                raise ValueError(f"バージョン文字列に無効な文字が含まれています: {version_string}")
            raise

    def execute_build_script(self, script_path: Optional[str] = None, 
        ...
        try:
            ...
            error_msg = f"ビルドスクリプト実行中に予期しないエラーが発生しました: {str(e)}"
            logger.error(error_msg)
            if progress_callback:
                progress_callback(f"エラー: {error_msg}")
            return False
