...
class TrayManager:
        ...
        try:
            if self._has_update != has_update:
                ...
                if self._icon is not None:
                    ...
                    self.logger.info(f"アイコン状態を更新しました: has_update={has_update}")
    ...
    def _create_icon_image(self, has_update: bool) -> Image.Image:
        ...
        if has_update:
            # 更新ありの場合は赤い円
            draw.ellipse([4, 4, 28, 28], fill=(255, 0, 0, 255), outline=(128, 0, 0, 255))
            # 感嘆符を描画
            draw.text((13, 8), "!", fill=(255, 255, 255, 255))
        else:
            # 通常時は青い円
            draw.ellipse([4, 4, 28, 28], fill=(0, 100, 200, 255), outline=(0, 50, 100, 255))
            # チェックマークを描画
