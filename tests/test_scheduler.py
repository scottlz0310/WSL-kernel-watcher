...
class TestScheduler:
    ...
    @pytest.fixture
    def scheduler(self, mock_config, mock_github_client, mock_wsl_utils, mock_notification_manager):
    ...
    def test_start_monitoring_failure(self, scheduler):
        """監視開始の失敗テスト"""
        with patch.object(scheduler.scheduler, 'start', side_effect=Exception("Test error")):
            result = scheduler.start_monitoring()
    ...
    def test_stop_monitoring_failure(self, scheduler):
        ...
        with patch.object(scheduler.scheduler, 'shutdown', side_effect=Exception("Test error")):
            result = scheduler.stop_monitoring()
    ...
    def test_check_for_updates_new_version_available(self, scheduler, mock_github_client, 
                                                   mock_wsl_utils, mock_notification_manager, 
                                                   sample_release):
        """新しいバージョンが利用可能な場合の更新チェックテスト"""
        # モックの設定
        mock_github_client.get_latest_stable_release.return_value = sample_release
        mock_wsl_utils.get_current_kernel_version.return_value = "5.15.89.1-microsoft-standard-WSL2"
    ...
    def test_check_for_updates_current_version_latest(self, scheduler, mock_github_client, 
                                                    mock_wsl_utils, mock_notification_manager, 
                                                    sample_release):
        """現在のバージョンが最新の場合の更新チェックテスト"""
        # モックの設定
        mock_github_client.get_latest_stable_release.return_value = sample_release
        mock_wsl_utils.get_current_kernel_version.return_value = "5.15.90.1-microsoft-standard-WSL2"
    ...
    def test_check_for_updates_current_version_newer(self, scheduler, mock_github_client, 
                                                   mock_wsl_utils, mock_notification_manager, 
                                                   sample_release):
        """現在のバージョンが最新より新しい場合の更新チェックテスト"""
        # モックの設定
        mock_github_client.get_latest_stable_release.return_value = sample_release
        mock_wsl_utils.get_current_kernel_version.return_value = "5.15.91.1-microsoft-standard-WSL2"
    ...
    def test_check_for_updates_github_api_failure(self, scheduler, mock_github_client, 
    ...
    def test_check_for_updates_wsl_failure(self, scheduler, mock_github_client, 
                                         mock_wsl_utils, mock_notification_manager, 
                                         sample_release):
    ...
    def test_check_for_updates_notification_failure(self, scheduler, mock_github_client, 
                                                   mock_wsl_utils, mock_notification_manager, 
                                                   sample_release):
        """通知表示失敗時の更新チェックテスト"""
        # モックの設定
    ...
    def test_check_for_updates_exception_handling(self, scheduler, mock_github_client, 
                                                 mock_notification_manager):
        """例外発生時のエラーハンドリングテスト"""
        # モックの設定 - _perform_integrated_update_check内で例外が発生するようにする
        with patch.object(scheduler, '_perform_integrated_update_check', side_effect=Exception("Test error")):
    ...
    def test_extract_version_from_kernel(self, scheduler):
        ...
        # 標準的なWSLカーネルバージョン
        version = scheduler._extract_version_from_kernel("5.15.90.1-microsoft-standard-WSL2")
        assert version == "5.15.90.1"
    ...
    def test_handle_check_error_github_api_error(self, scheduler, mock_notification_manager):
    ...
    def test_update_check_interval_with_monitoring_restart(self, scheduler):
        ...
        with patch.object(scheduler, 'stop_monitoring') as mock_stop, \
    ...
    def test_perform_integrated_update_check_success(self, scheduler, mock_github_client, 
                                                    mock_wsl_utils, sample_release):
        """統合更新チェック成功テスト"""
        # モックの設定
        mock_github_client.get_latest_stable_release.return_value = sample_release
        mock_wsl_utils.get_current_kernel_version.return_value = "5.15.89.1-microsoft-standard-WSL2"
    ...
    def test_perform_integrated_update_check_github_failure(self, scheduler, mock_github_client):
    ...
    def test_perform_integrated_update_check_wsl_failure(self, scheduler, mock_github_client, 
                                                       mock_wsl_utils, sample_release):
        """統合更新チェック（WSL失敗）テスト"""
        mock_github_client.get_latest_stable_release.return_value = sample_release
        mock_wsl_utils.get_current_kernel_version.return_value = None
    ...
    def test_process_update_check_result_update_available_notify(self, scheduler, 
                                                               mock_notification_manager):
        ...
        with patch.object(scheduler, '_execute_notification_logic', return_value=True) as mock_notify:
    ...
    def test_execute_notification_logic_success(self, scheduler, mock_notification_manager):
        """通知ロジック実行成功テスト"""
        update_info = {
            "current_version": "5.15.89.1",
            "latest_version": "5.15.90.1",
        ...
        with patch.object(scheduler, '_ensure_notification_click_handler') as mock_handler:
    ...
    def test_execute_notification_logic_failure(self, scheduler, mock_notification_manager):
        """通知ロジック実行失敗テスト"""
        update_info = {
            "current_version": "5.15.89.1",
            "latest_version": "5.15.90.1",
    ...
    def test_ensure_notification_click_handler(self, scheduler, mock_notification_manager):
        """通知クリックハンドラー設定テスト"""
...
class TestSchedulerIntegration:
    ...
    def test_error_recovery(self, real_config):
        ...
        # 最初のチェックでエラーが発生
        mock_github_client.get_latest_stable_release.side_effect = Exception("Network error")
