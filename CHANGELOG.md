...
### Changed
- 設定画面の入力変換・保存、ランチャープリセット適用、Docker の Gateway 検出、MCP Resource URI 取得を `MainWindow.xaml.cs` から抽出した（#267、epic #262）。入力途中の設定は既存値を保持し、予期しない例外は握り潰さない。`MainWindow.xaml.cs` は 1,793 行から 1,679 行になり、行数チェックの上限も同じ値へ下げた
...
- レビュー起動の手続きと分岐を `Services/ReviewStartCoordinator.cs` へ抽出した（#264、epic #262）。多重起動の抑止（起動中フラグと `IsRunning` の二段ガード）・Auto-Pause（#147）の評価と手動 / 自動での分岐・レートリミット snapshot の取得・セッション生成・自動起動（#254）の判定と記録が `MainWindow.xaml.cs` にあり、カバレッジ計測の対象外だった。起動可否と理由を `ReviewStartResult` で返し、`ContentDialog` の表示とライブログウィンドウの生成だけを code-behind に残す。Auto-Pause override の確認は delegate として受け取る（確認ダイアログの表示中も起動中フラグを保持する必要があり、二相 API では #147 で修正した ContentDialog 多重表示が再発するため）。挙動は変えていない。`MainWindow.xaml.cs` は 2,058 行から 1,964 行になり、行数チェックの上限も同じ値へ下げた
...
- レートリミット更新経路を `Services/RateLimitRefreshCoordinator.cs` へ抽出した（#266、epic #262）。監視対象エージェントの列挙と経路別（codex は App Server、それ以外は statusline フックのローカルファイル）の snapshot 取得、取得失敗の分類、旧形式 snapshot（#168）の検出、MCP `ratelimit://` リソースの取得、Auto-Pause gate（#147/#167）の再評価が `MainWindow.xaml.cs` にあり、カバレッジ計測の対象外だった。個々のサービス（`RateLimitSnapshotResolver` / `RateLimitSnapshotService` / `RateLimitFreshnessPolicy`）はテスト済みだったが、それらを束ねて「何を表示し何を警告するか」を決める部分だけが未検証だった。表示する一覧・提示する警告・旧形式の警告文を 1 つの結果型で返し、`ObservableCollection` への反映と `InfoBar` ・ダイアログの表示だけを code-behind に残す。Auto-Pause gate の所有者は #264 と揃えて Coordinator 側に置いた。挙動は変えていない。`MainWindow.xaml.cs` は 1,953 行から 1,793 行になり、行数チェックの上限も同じ値へ下げた
- レートリミット更新でユーザーへ提示する文言の組み立てを `Helpers/RateLimitAlertFormatter.cs` へ抽出した（#266）。codex の取得不可理由の出し分け（#174）・statusline 未拡張の案内・読み取り失敗・旧形式 snapshot の警告を持つ
...
- review event を受信した時点で reviewer 側エージェントを自動起動できるようになりました（#254）。設定「レビュー自動開始」で有効にすると、キューに載った PR のレビューがクリックなしで始まります。**既定は無効**で、無効時の挙動は従来と変わりません。無人で起動するため、Auto-Pause 中と他のレビュー実行中は自動起動せず、理由を「Recent activity」に記録します
- 起動時に稀に異常終了する問題を修正しました（#229）。トレイポップアップ用 Window の生成を XAML ロードの外へ移し、失敗しても通知をバルーンへフォールバックするようにしています
- PC 起動直後（Docker / mcp-gateway の初期化前）でも購読が自動復帰するようになりました（#236）。接続拒否のときだけ 5 秒間隔・上限 5 分で待機し、待っても解決しない認証・DNS・TLS のエラーは従来どおり早期に確定します
- メイン画面の「Recent activity」が最新行へ自動スクロールするようになりました（#232）。過去ログを読むために上へスクロールしている間は引き戻しません
- キューが空のときに約 60 秒ごとに出ていた誤った警告が出なくなりました（#230）
- `.cmd` / `.bat` 形式のランチャーの出力が文字化けしなくなりました（#231）
- ランチャーの引数を編集して「カスタム」になったスロットでも Auto-Pause が機能するようになりました（#233）。あわせて、Auto-Pause の対象外になっているスロットをメイン画面に表示するようにしました

**必要な構成**

v0.7.0 から変更ありません。mcp-resource-subscriber v0.6.0 以降 / thread-owl v0.4.0 以降 / review-raven v0.2.0 以降 / mcp-gateway v0.10.0 以降と組み合わせて動作します。

**既知の問題**

v0.7.0 で挙げた 5 件（#229 / #230 / #231 / #232 / #233）はすべて解消しました。未修正の既知の問題はありません。

ただし、新規機能である「レビュー自動開始」（#254）は実機での E2E 検証を行っていません。`MainWindow.xaml.cs` はカバレッジ計測の対象外のため、自動起動の経路は単体テストの範囲外です。有効にする場合は、まず在席時に動作を確認してください。

### Added
- review event の受信時に reviewer 側エージェントを自動起動する設定「レビュー自動開始」を追加した（#254）。従来は queue に event が載ってから起動まで必ずユーザーのクリックが 1 回挟まり、席を外しているとレビューサイクル全体が止まっていた。起動判断は実質的に定型（キューに載った＝レビューする）のため、opt-in で自動化できるようにした。**既定は無効**で、無効時の挙動は従来と完全に同じ（通知のみ）。起動経路は「レビューする」と同じ `ExecuteReviewAsync` を再利用し、自動起動専用の経路は作らない。無人で走るため、Auto-Pause（#147）が Paused のときは強行確認ダイアログを出さずに見送り、別のレビューが実行中のときも起動しない。見送った理由・自動起動した事実・自動起動時のエラーはいずれも「Recent activity」に記録する。見送ったイベントはイベント一覧に残るため手動起動で拾える（実行終了後の自動リトライは行わない）。自動起動したイベントの通知は「自動でレビューを開始しました」と文言を変え、押しても同時実行抑止で弾かれるだけの「レビューする」ボタンを出さない。イベント受信から起動可否までの判定は `ReviewAutoStartPolicy` として UI から切り離した
...
### Fixed
- メイン画面の「Recent activity」が最新行へ自動スクロールするようになった（#232）。`OnLogAppended` は行を追加するだけで `ScrollIntoView` に相当する処理を持たず、リストは常に起動直後の数行を表示し続けていた。データは追加されているのにビューが動かないため、「Recent activity」という名前に反して現在の動作状況が一切見えなかった。ただし無条件追従にはせず、`LogFollowPolicy` が ScrollViewer の位置から判定して**末尾付近にいるときだけ**追従する。ユーザーが過去ログを読むために上へスクロールしている間は引き戻さず、末尾へ戻れば追従が再開する。判定は行を追加する前の位置で行う（追加後は `ScrollableHeight` が伸び、末尾にいた状態が末尾付近でなくなるため）。ライブログウィンドウ側（`AgentExecutionWindow`、現状は無条件追従）の挙動を揃えるかは別途判断する
- 空のレビューキュー（`[]`）を「Malformed or unsupported」警告として記録しなくなった（#230）。`ProcessReviewEventPayloadAsync` はパース結果が 0 件であることだけを見て一律に不正 payload と扱っていたため、**キューに候補が無いという最頻の正常状態**が、再購読サイクルごと（既定 60 秒）に警告として記録され続けていた。1 日あたり 1,400 行以上の偽警告がログを占め、実際に payload が壊れたときの警告が意味を失っていた。`ReviewEventParser.Parse` が `ReviewEventParseResult` を返すようにし、「解釈できて 0 件（空キュー）」と「JSON として解釈できない・想定スキーマから 1 件も構築できない」を `Status` で区別する。併せて、毎サイクル生 payload を再掲していた `InitialText payload received: []` も、イベントが 1 件以上あるときだけ記録するようにした
- `.cmd` / `.bat` ランチャーの出力が文字化けする問題を修正した（#231）。`cmd.exe` は自身のメッセージ（「内部コマンドまたは外部コマンド…」等）を OEM コードページで書き出すが、その先で起動される実エージェントは UTF-8 を書き出すため、1 本のハンドルに 2 種類のエンコーディングが混在する。読み取り側が UTF-8 固定だったため、**失敗時に原因を示す唯一のメッセージが読めなくなっていた**。`ReviewLauncherService` は `Encoding.Latin1` でストリームを開いてバイト値を保存したまま行に分割し、`ProcessOutputDecoder` が行ごとに UTF-8 として厳密にデコードして、不正バイト列だった行のみ OEM コードページで再デコードする。妥当な UTF-8 は往復して元の文字列に戻るため、UTF-8 のみを出力するネイティブ実行形式の経路は影響を受けない
...
- アプリ内から mcp-gateway の初回認証（device flow）を実行できるようになりました（#183）
- 購読停止中にレビューを登録すると、購読の開始を確認してから enqueue するようになりました（#209）
- アプリ停止中に queue へ入った既存のレビュー候補も、購読開始時に通知されるようになりました（#210）
- レビューイベント受信時にアプリが異常終了する問題を修正しました（#199）
- トレイの右クリックメニューがダークテーマに追従し、購読状態に応じて活性制御されるようになりました（#202）

**既知の問題**

本リリース時点で未修正の問題があります。次回以降で対応します。
...
- 購読開始の完了を待てる `McpSubscriptionService.StartAsync` を追加した（#208、epic #185）。従来の `Start()` は `void` で購読ループを投げて即座に戻るため、呼び出し側は購読が `Running` に到達したか `Error` で終わったかを知る手段がなく、「購読を開始してからレビューを登録する」導線（#209）を UI 側で実装できなかった。新 API は状態遷移を `StateChanged` だけで観測し（ポーリングしない）、`Started` / `Failed` / `TimedOut` / `Cancelled` を区別した `SubscriptionStartResult` を返す。既に `Running` なら即座に成功を返し、`Starting` 中の多重呼び出しは進行中の開始結果を待って購読プロセスを二重起動しない。停止処理中（`Stopping`）の呼び出しは、新しいループを起動した直後に `StopAsync` の後始末が `State = Stopped` を書き込んで状態と実体が食い違うのを避けるため、開始せずに失敗を返す。この判定は開始可否の確認と `State` / `_stopCts` / `_loopTask` の差し替えを同一の排他区間で行うことで実現しており、`StopAsync` の `Stopping` 遷移も同じロック配下に置いて開始・停止を直列化している（確認と生成が分かれていると、その隙間に停止が割り込んだ場合に停止中の購読ループを起動してしまい、`StopAsync` が新しいループを待ち続ける）。購読ループ側の `Running` / `Error` 遷移にも同じ規約を適用し、停止処理中は状態を書き戻さない（`StopAsync` はロックを解放してから cancel するため、その後にループが `Error` を書き戻すと、通知を受けた側が `Stopping` ガードをすり抜けて新しいループを開始でき、`StopAsync` が捕捉済みの `loopTask` に含まれない実体だけが残る）。タイムアウトは DI 可能（既定 30 秒）。既存の `Start()` 呼び出し経路（起動時の自動開始・Start / Retry ボタン・トレイメニュー）は変更していない。なお mcp-gateway の認証要求はこの結果では表現しない。購読開始の可否は preflight（subscriber の `--help`）だけで決まり、gateway への接続は `Running` 到達後の購読プロセスが初めて行うため、認証要求は購読ループが `Error` へ落ちた時点の `IsAuthenticationRequired` に現れる
...
### Changed
- PR CI の壁時計を計測可能にし、重複処理を削減した（#220、epic #184）。品質ゲート（`dotnet format`、build、unit test、coverage 閾値 line / branch / method 各 80%、Roslyn analyzer、StyleCop、Roslynator、SecurityCodeScan、CodeQL、MSI MajorUpgrade 検証）と target SDK / runtime / toolchain は一切変更していない。削減内容は次の 7 点。(1) Chocolatey による Windows SDK install を削除した。これは TFM が `net10.0-windows10.0.22621.0` だった時期の windows-2025 対応で、同じ PR 内で TFM が `10.0.26100.0` へ移行した時点で不要になっていた（`windows-sdk-10.1` は 26100 を提供せず、`WindowsSdkPackageVersion` は NuGet の参照アセンブリで解決される）。(2) solution が `net10.0-windows` のみを対象とするため、`8.0.x` SDK の導入を削除した。(3) NuGet global packages folder を `global.json` / `nuget.config` / `Directory.Packages.props` / project file の hash をキーにキャッシュし、共通の composite action へ集約した。(4) coverage の `dotnet test` を種別ごとの 3 回実行から、`ThresholdType=line,branch,method` による 1 回実行へまとめた。(5) `Code Analysis` step の再ビルドを `Build solution` へ統合した（差分は `TreatWarningsAsErrors` / `RunCodeAnalysis` の指定のみで、両方を 1 回目のビルドへ渡して検出範囲を保つ）。(6) 非推奨の `dotnet-format` グローバルツール導入を削除し、lefthook の pre-commit と同じ SDK 組み込みの `dotnet format` を使うようにした。(7) `pull_request` の後続 push で先行 run を自動 cancel するようにした（`main` / `develop` への push は検証記録を残すため対象外）。あわせて job / step / 壁時計の所要時間を job summary へ記録する `timing-report` job と `scripts/report-ci-timings.ps1` を追加し、baseline と Phase 1 E2E の時間予算を [`docs/ci-wall-clock.md`](docs/ci-wall-clock.md) に文書化した。実測は同一 HEAD の 5 回で中央値 368 秒（baseline 518 秒に対し 29.0% 短縮、warm cache のみでは 355 秒 / 31.5%）。#220 の目標である 30% 短縮には届いていないため、残りの最適化は #220 で継続する。critical path の `security-scan` は CodeQL 関連が約 250 秒を占めるが、`build-mode: none` は XAML codegen 等の生成コード 8,204 LOC と依存パッケージ由来ソースが解析対象から欠落し CodeQL の解析完全性が下がるため不採用とした（検証結果は `docs/ci-wall-clock.md` に記録）
- メイン画面の購読操作ボタンを「購読を開始」「購読を停止」「再試行」に日本語化し、トレイメニューおよび購読開始確認ダイアログと用語を統一した（#211、epic #185）

### Fixed
- トレイの右クリックメニューがシステムのダークテーマに追従せず、購読状態に応じた活性制御もない問題を修正した（#202）。原因は、`TaskbarIcon.ContextFlyout` に置いた XAML `MenuFlyout` が XAML として描画されていなかったことにある。`H.NotifyIcon` は既定の `ContextMenuMode.PopupMenu` でメニューを Win32 のネイティブメニューへ変換して表示するため、`ThemeResource` による配色も `MenuFlyoutItem.IsEnabled` も反映されない。`ContextMenuThemeMode`（`System` / `Dark` いずれも）や `XamlUICommand.CanExecuteRequested` を設定しても実機では効果がなく、`ContextMenuMode.SecondWindow` はメニュー自体が表示されなかった。そこで `MenuActivation="None"` でライブラリ側のメニュー表示を止め、`RightClickCommand` から自前の Win32 ポップアップメニュー（`TrayContextMenu`）を表示するようにした。メニューは表示のたびに現在の購読状態で組み直し、購読中は「購読を開始」を、停止中は「購読を停止」を `MF_GRAYED` で無効化する（メインウィンドウの Start / Stop ボタンと同じ `SubscriptionControlAvailability` の判断を共有する）。テーマ追従は uxtheme の `SetPreferredAppMode(AllowDark)` で有効化し、表示直前に `RefreshImmersiveColorPolicyState` と `FlushMenuThemes` を呼ぶことでアプリ再起動なしにライト／ダークの切り替えへ追従する。これらは非公開の ordinal API のため、呼び出しに失敗しても機能は落とさずライト表示のまま継続する
- subscriber 子プロセスのエラーハンドリング不備 2 件を修正した（#201）。(1) `McpSubscriptionService.PreflightCheckAsync` は subscriber の起動に失敗した場合しか `LastError` を設定せず、起動できて終了コードが非 0 だった場合は空のまま `State = Error` へ遷移していたため、トレイのバルーン通知が「接続エラー: 」とだけ表示され原因を判断できなかった。終了コードと出力の要約（stderr、空なら stdout）を含むメッセージを設定するようにした。(2) `McpLoginService.CheckSubscriberVersionAsync` は `RedirectStandardError = true` としながら stderr を読み出さずに `WaitForExitAsync` を待っていたため、subscriber がパイプバッファ（既定 4KB 程度）を超える stderr を出力すると子プロセスが書き込みでブロックし、バージョン確認が返らなくなりうる状態だった。stderr も読み出し、バージョン判定に失敗した場合はその要約をメッセージへ含める。同一パターンの `EnqueueReviewService.CheckSubscriberVersionAsync` も併せて修正した。UI・ログへ出す出力は `SecretMasker` と `AnsiControlSanitizer` を通す共通ヘルパー `ProcessOutputSummarizer` に集約し、`ReviewLauncherService` の同等実装を置き換えた
- mcp-gateway のログインダイアログが処理完了後も「認証を開始しています...」のまま残る問題を修正した（#200）。`ContentDialog.ShowAsync()` は呼び出し直後には開き終わっておらず、その途中で `Hide()` が到達すると無視されるため、ログインが速く完了するとダイアログが閉じず、ユーザーがキャンセルを押すまで成功・失敗の結果が表示されなかった。`ContentDialog.Opened` までクローズ要求を保留する `DeferredDialogCloseGate` を追加し、開き終わってから閉じるようにした。`DispatcherQueue.TryEnqueue` が失敗した場合はログに記録する。あわせて、値が届く前にラベルだけが並んで何を待っているのか分からなかったため、「承認 URL:」「認証コード:」のラベルを対応する値と同時に表示するようにした
